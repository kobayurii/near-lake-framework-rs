...
impl Client for FastNearClient {
    async fn get_object(
        ...
        let result = self.client.get(
                &format!("{}/v0/block/{}", self.endpoint, block_height)
            )
                .send()
                .await?
                .json()
    ...
    async fn list_objects(
        ...
        Ok((start_after..=start_after + 1000).collect())
...
> {
    let sm = lake_s3_client
        .get_object(block_height).await?;
    Ok(sm.shards[shard_id as usize].clone())
}
